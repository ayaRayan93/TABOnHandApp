...
public class InvoiceContentProvider2BasicData extends ContentProvider {

    // database
    private DataBaseHelper database;//=CustomerContentProvider.database;
    private Context context;
    // used for the UriMacher
    public InvoiceContentProvider2BasicData() {

    }
    public InvoiceContentProvider2BasicData(DataBaseHelper database) {
        this.database=database;
    }
    public InvoiceContentProvider2BasicData(Context context) {
...
//        cursor.setNotificationUri(getContext().getContentResolver(), uri);
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        long id = 0;
        try {
            id = sqlDB.insert(InvoiceTable2BasicData.InvoiceTable, null, values);
        }
        catch(Exception e){
            throw new IllegalArgumentException("Unknown URI: " + uri);
        }
