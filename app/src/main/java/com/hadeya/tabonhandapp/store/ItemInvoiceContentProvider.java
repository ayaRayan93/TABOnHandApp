...
public class ItemInvoiceContentProvider extends ContentProvider {
    // database
    private DataBaseHelper database;//=CustomerContentProvider.database;
    private Context context;
    // used for the UriMacher
    public ItemInvoiceContentProvider() {

    }
    public ItemInvoiceContentProvider(DataBaseHelper database) {
        this.database=database;
    }
    public ItemInvoiceContentProvider(Context context) {
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        // Set the table
        queryBuilder.setTables(ItemInvoiceTable.ItemInvoiceTable);
...
//        cursor.setNotificationUri(getContext().getContentResolver(), uri);
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        long id = 0;
        try {
            id = sqlDB.insert(ItemInvoiceTable.ItemInvoiceTable, null, values);
        }
        catch(Exception e){
            throw new IllegalArgumentException("Unknown URI: " + uri);
        }
